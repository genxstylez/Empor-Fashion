...
class CartItem(models.Model):
    ...
    discount = models.ForeignKey(Discount, verbose_name=_('Discount'), related_name='cart_items', null=True, blank=True)
    ...
    def get_discount_price(self):
        if self.discount:
            if self.discount.amount:
                return int(self.discount.amount)
            else:
                return int(self.discount.percentage/100 * self.product.price)
        else:
            return 0
...
class ArchivedCartItem(models.Model):
    ...
    discount = models.ForeignKey(Discount, verbose_name=_('Discount'), related_name='archived_cart_items', null=True, blank=True)
    quantity = models.PositiveIntegerField(_('Quantity'), default=1)
    total = models.PositiveIntegerField(_('Total'), default=0)
    created_at = models.DateTimeField(_('Created at'))
    last_modified = models.DateTimeField(_('Last modified'))
